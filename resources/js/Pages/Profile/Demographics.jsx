...
export default function Demographics({ demographicsSettings = {} }) {
    // 0. Visibilitas Seksi Demografi (On/Off)
    const showKpiCards = demographicsSettings?.show_kpi_cards !== '0' && demographicsSettings?.show_kpi_cards !== false;
    const showLandUse = demographicsSettings?.show_land_use !== '0' && demographicsSettings?.show_land_use !== false;
    const showProfessions = demographicsSettings?.show_professions !== '0' && demographicsSettings?.show_professions !== false;
    const showAgeGroups = demographicsSettings?.show_age_groups !== '0' && demographicsSettings?.show_age_groups !== false;
    const showEducation = demographicsSettings?.show_education !== '0' && demographicsSettings?.show_education !== false;

    const hasAnyVisibleSection = showKpiCards || showLandUse || showProfessions || showAgeGroups || showEducation;
    ...
    return (
        <AppLayout>
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 space-y-12 sm:space-y-16">
                ...
                {/* ============================================================ */}
                {(showProfessions || showAgeGroups) && (
                    <div className={`grid gap-8 ${showProfessions && showAgeGroups ? 'grid-cols-1 lg:grid-cols-2' : 'grid-cols-1'}`}>
                ...
                {/* 6. FALLBACK JIKA SEMUA SEKSI DI-NONAKTIFKAN OLEH ADMIN */}
                {!hasAnyVisibleSection && (
                    <div className="relative overflow-hidden rounded-2xl border border-dashed border-zinc-300 dark:border-zinc-700 bg-white dark:bg-zinc-900/60 p-12 text-center shadow-xs">
                        <div
                            className="absolute inset-0 pointer-events-none opacity-[0.03] dark:opacity-[0.05] bg-repeat"
                            style={{
                                backgroundImage: `url("${BATIK_DEMOGRAFI_PATTERN}")`,
                        ...
                        <div className="relative z-10 max-w-md mx-auto space-y-4">
                            <div className="h-16 w-16 mx-auto rounded-2xl bg-amber-50 dark:bg-amber-950/40 text-amber-600 dark:text-amber-400 flex items-center justify-center border border-amber-200/60 dark:border-amber-900/40 shadow-xs">
                                <PieChart className="h-8 w-8" />
                            </div>
                            <div>
                                <h3 className="text-lg font-bold text-zinc-900 dark:text-white">
                                    Informasi Statistik Sedang Diperbarui
                                </h3>
                                <p className="text-sm text-zinc-500 dark:text-zinc-400 mt-1.5 leading-relaxed">
                                    Seksi data kependudukan dan demografi Desa Karangwungu saat ini sedang dalam proses sinkronisasi berkala oleh pemerintah desa. Silakan kembali beberapa saat lagi.
                                </p>
                            </div>
                        </div>
                    </div>
